...
describe('Producer unit tests', () => {
	...
	//////////////////////////////////////////////////////////////////////////////
	// the redis connection is closed byond this point so only expected failures
	// should be tested beyond this point
	//////////////////////////////////////////////////////////////////////////////

	it('should reject on error when push is called with no connection to redis', (done) => {
		// this is a hack to try to get better code coverage
		// it relies on the client to initiate connection loss asynchronously
		// before the producer knows the connection has disconnected in
		// order to throw a redis error
		client.quit();
		producer.push(randomEvent())
			.then(() => {
				done('push should throw redisError');
			})
			.catch((error) => {
